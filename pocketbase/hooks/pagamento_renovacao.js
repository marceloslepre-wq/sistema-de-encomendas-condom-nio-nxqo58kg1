...
routerAdd('POST', '/backend/v1/pagamento/pix/criar', (e) => {
...
routerAdd('GET', '/backend/v1/pagamento/pix/status/{payment_id}', (e) => {
  const auth = e.requestInfo().authRecord
  if (!auth) {
    return e.forbiddenError('Não autenticado.')
  }
    ...
    if (payRes.statusCode === 200 && payJson.id) {
      ...
      if (mpStatus === 'approved') {
        ...
        if (!localPag || localPag.getString('status') !== 'approved') {
          ...
          const effCondoId = condoId || (licAntiga ? licAntiga.getString('condo_id') : userCondoId)
